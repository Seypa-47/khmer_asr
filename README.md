...
`src/train_qwen_lora.py` and `notebooks/qwen_lora_finetune.ipynb` define a separate **student-owned LoRA fine-tuning run** using only the saved 531 training and 124 validation clips. The Qwen project's official fine-tuning helper is pinned by commit. The first one-epoch T4 run scored **14.45% validation CER and 33.12% validation WER**, slightly worse than the publisher checkpoint on both measures. See `results/diagnostic/qwen_lora_validation_v1.json`. That adapter is not selected for the held-out test or the final presentation. A local replication saves weights outside Git because the Colab Drive mount failed during this run. Any further experiment must be selected by validation WER before its single held-out test evaluation.
