...
`diagnostic/qwen_lora_expanded_v2.json` contains the completed 883-training-clip experiment: both epochs' public validation predictions, rescored metrics, subgroup error counts, configuration, checkpoint hashes, and limitations. Epoch 1 scored 14.52% CER / 32.26% WER; epoch 2 worsened to 15.08% / 34.02%. The selected epoch is only the best within this run, not an improvement over the recorded public baseline. The original 124 validation and 114 test row lists are preserved in `diagnostic/qwen_expanded_train_split.json`; no Qwen test evaluation was run. The notebook documents historical reproduction; more epochs on this recipe are not recommended.
